...
const ImageInList = ({
  ...
  if (has_origin && record.is_origin === 1) {
    console.log('has_origin', has_origin, record.is_origin, record.path)
  }
  return (
    <div
      className={`img-container ${record.status === 1 ? 'clicked' : ''}`}
      key={index}
      style={{
        backgroundColor: has_origin && record.is_origin === 1 ? '#ff0000' : '',
      }}
    >
