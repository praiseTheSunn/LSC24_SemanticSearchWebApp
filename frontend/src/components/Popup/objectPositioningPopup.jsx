...
const ObjectPositionPopup = ({ showPopup, setResult, setCacheResult }) => {
  ...
  const handleQuery = () => {
    ...
    for (const obj of selectedObjects) {
      ...
      const new_element = {
        object_name:
          obj.icon.name.charAt(0).toUpperCase() + obj.icon.name.slice(1),
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col">
        <button
          className="bg-red text-white rounded-[3px] w-[50px] h-[30px] ml-[3px]"
          onClick={(e) => handleClear(e)}
        >
          Clear
        </button>
        <button
          className="bg-blue text-white rounded-[3px] mt-2 w-[50px] h-[30px] ml-[3px]"
