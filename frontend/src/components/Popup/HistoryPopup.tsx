...
import { type CSSProperties, type Dispatch, type SetStateAction, useCallback, useEffect, useMemo, useRef } from "react";
import type { MeasuredCellParent } from "react-virtualized/dist/es/CellMeasurer";
import type { SearchTermType } from "../../types/search";

const HistoryPopup = ({setSearchTerms, setDisplayedFilters} 
  : 
  {
  ...
  const setQueryPayload = (payload: any) => dispatch(appActions.setQueryPayload(payload));
  ...
  const handleHistoryClick = (index: number) => {
    ...
    setQueryPayload({ parsedQuery, mode: queryPayload.mode, model: queryPayload.model });
    setDisplayedFilters((historyItem.query.split(' | ')).map(filter => ({ category: filter.split(': ')[0], value: filter.split(': ')[1], status: 1 })));
    
    trigger({ text_query: parsedQuery, mode: queryPayload.mode, model: queryPayload.model })
    .then(() => {
      setSearchTerms(filters.map(filter => ({ category: filter.split(': ')[0], value: filter.split(': ')[1], status: 1 })));
    });
  };
