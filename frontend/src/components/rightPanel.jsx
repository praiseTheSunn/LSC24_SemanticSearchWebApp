...
const RightPanel = ({ query, filters, setDisplayedFilters }) => {
  ...
  const { similarPopUp, setSimilarPopUp, setNeighborPopUp, setLoadingPopUp } =
    usePopUp()
  ...
  useEffect(() => {
    if (query !== '') {
      ...
      // First HTTP request
      const response2 = imageService
        .getImages(query) // change query here
        .then(async (response2) => {
          // Convert the byte data to a base64-encoded string
          const urls = response2.data.image_files
          const imageDataUrls = []
  ...
  const fetchImagesToCopy = (index, max, copy) => {
    if (index < max) {
      if (imageUrls[index] === undefined) return copy
  ...
  return (
    <div className="right-content-container">
      {similarPopUp && (
        <SinglePopup
          viewImage={viewImage}
          openSinggleImage={openSinggleImage}
        />
      )}
          ...
          {selectedImages.map((record, index) => (
            <div className="thumbnail-wrapper" key={index}>
              ...
              <img
                ...
                onClick={() => removeSelectedImage(record.url)}
              />
            </div>
          ))}
        </div>
