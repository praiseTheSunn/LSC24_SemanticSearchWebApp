...
METADATA_DIR = "/home/pc/LSC24_SemanticSearchWebApp/backend/data/object_encoding/MVK_brush"
    ...
    if filename.endswith(".json"):
        ...
        with open(os.path.join(METADATA_DIR, filename), 'r') as f:
            ...
            for keyframe_order, key in enumerate(keys, start=1):
                ...
                name = f"MVK/{video_order}/{context_order:05d}/{keyframe_order:05d}"
                try:
                    id = id_mapping.loc[name, 'keyframe_id']
                except:
                    continue
                body = {
                    "local_object_encoding": data[f"{key:05d}.jpg"]["object_encoding"]["local"],
                    "global_object_encoding": data[f"{key:05d}.jpg"]["object_encoding"]["global"],
                    "local_color_encoding": data[f"{key:05d}.jpg"]["color_encoding"]["local"],
                    "global_color_encoding": data[f"{key:05d}.jpg"]["color_encoding"]["global"]
                }
