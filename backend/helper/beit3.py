...
def to_image_tokens(image_path):
...
def calc_img_embedding(image_path):
    img_tokens = to_image_tokens(image_path)
    with torch.no_grad(), torch.cuda.amp.autocast():
        img_embedding = model(image=img_tokens, only_infer=True)
...
def calc_both_embedding(image_path, text, tokenizer):
    img_tokens = to_image_tokens(image_path)
    text_tokens, padding_mask = to_text_tokens(text, tokenizer)
