#!/usr/bin/env bash
set -euo pipefail

# Resolve repo root (works even if script is symlinked)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"                    # Edit here when you move the script to another place

export PYTHONPATH="$REPO_ROOT/libs"

cd "$REPO_ROOT"

# python -m milvus.utils.create_lsc24 --dataset vbs25_v3c --no-milvus --force

# python -m milvus.utils.create_lsc24 --dataset vbs25_mvk --no-es --force

python -m milvus.utils.create_lsc24 --dataset vbs25_lhe --no-es --force

# python -m milvus.utils.create_lsc24 --no-es --force

# python -m milvus.utils.create_lsc24 --no-es --force
